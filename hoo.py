...
# --- Configuration ---
TOKEN = '6697645670:AAEd4NHwNdmNc2wXz_0jKjCc_-DoBmfkoqw'
OWNER_ID = 6231439063
ADMIN_ID = 6231439063
YOUR_USERNAME = '@Expert_Programmer'
UPDATE_CHANNEL = 'https://t.me/@Expert_Programmer'
...
# File upload limits
FREE_USER_LIMIT = 2  # free users can host up to 2 bots
...
# --- Pending approval queue (admin approves before hosting) ---
# token -> dict(user_id, chat_id, message_id, file_name, file_path, file_ext)
pending_approvals = {}

class _SimpleMsg:
    """Minimal message-like object for bot.reply_to compatibility."""
...
COMMAND_BUTTONS_LAYOUT_USER_SPEC = [
    ["📤 Upload File", "📂 Your Files"],
    ["⚡ Bot Speed", "📊 Statistics"],
    ["📞 Owner"]  # Added Send Command
]
...
def create_main_menu_inline(user_id):
    ...
    buttons = [
        ...
        types.InlineKeyboardButton('📂 Your Files', callback_data='check_files'),
...
def handle_zip_file(downloaded_file_content, file_name_zip, message):
    ...
    try:
        ...
        if not main_script_name:
            bot.reply_to(message, "❌ No `.py` script found in archive!"); return
...
def _logic_send_welcome(message):
    ...
    welcome_msg_text = (f"〽️ Welcome, {user_name}!\n\n🆔 Your User ID: `{user_id}`\n"
                        ...
                        f"🤖 Host & run Python (`.py`) script.\n"
                        f"   Upload single script or `.zip` archives.\n\n"
...
def handle_file_upload_doc(message):
    ...
    try:
        ...
        if file_ext == '.zip':
            ...
            logger.info(f"Saved zip to {file_path} (pending approval)")
            ...
            bot.send_message(chat_id, "✅ ZIP received. ⏳ Waiting for admin approval before extracting/hosting.")

            kb = types.InlineKeyboardMarkup()
            kb.row(
                types.InlineKeyboardButton("✅ Approve", callback_data=f"approve_{token}"),
                types.InlineKeyboardButton("❌ Reject", callback_data=f"reject_{token}")
            )
            admin_text = f"""📝 *Hosting Approval Request*
            ...
            for aid in admin_ids:
                try:
                    bot.send_message(aid, admin_text, parse_mode="Markdown", reply_markup=kb)
                except Exception as e:
                    logger.error(f"Failed to notify admin {aid} about approval request: {e}")
            ...
            # ⛔ Admin approval required before hosting/running
            token = uuid.uuid4().hex[:12]
            pending_approvals[token] = {
                "user_id": user_id,
                "chat_id": chat_id,
            ...
            # Notify uploader
            bot.send_message(chat_id, "✅ File received. ⏳ Waiting for admin approval before hosting/running.")

            # Send approval request to admins/owner
            kb = types.InlineKeyboardMarkup()
            kb.row(
                types.InlineKeyboardButton("✅ Approve", callback_data=f"approve_{token}"),
                types.InlineKeyboardButton("❌ Reject", callback_data=f"reject_{token}")
            ...
            # Do NOT auto-run here; will run on approve
...
def handle_approval_callback(call):
    """Admin approves/rejects pending hosting requests."""
    ...
    req = pending_approvals.get(token)
    if not req:
        bot.answer_callback_query(call.id, "⚠️ Request not found / expired.", show_alert=True)
        return
    ...
    # Remove from pending first to avoid double-approve
    ...
    if action == "reject":
        ...
        try:
            ...
            logger.error(f"Failed to delete rejected file {file_path}: {e}")

        bot.answer_callback_query(call.id, "❌ Rejected.", show_alert=True)
        try:
            bot.send_message(chat_id, f"❌ Your file `{file_name}` was *rejected* by admin.", parse_mode="Markdown")
        except Exception as e:
            logger.error(f"Failed to notify user about rejection: {e}")

        try:
            bot.edit_message_text(f"❌ Rejected: `{file_name}` (User `{target_uid}`)", call.message.chat.id, call.message.message_id, parse_mode="Markdown")
        except Exception:
            pass
        return
    ...
    try:
        bot.send_message(chat_id, f"✅ Your file `{file_name}` was *approved*. Starting hosting...", parse_mode="Markdown")
    except Exception as e:
        logger.error(f"Failed to notify user about approval: {e}")

    # Run based on type using existing functions
    msg_obj = _SimpleMsg(chat_id=chat_id, message_id=msg_id, user_id=target_uid)

    try:
        if file_ext == ".zip":
            with open(file_path, "rb") as f:
                content = f.read()
            handle_zip_file(content, file_name, msg_obj)
        elif file_ext == ".js":
            user_folder = get_user_folder(target_uid)
            handle_js_file(file_path, target_uid, user_folder, file_name, msg_obj)
        elif file_ext == ".py":
            user_folder = get_user_folder(target_uid)
            handle_py_file(file_path, target_uid, user_folder, file_name, msg_obj)
        else:
            bot.send_message(chat_id, "⚠️ Unsupported file type after approval.")
    except Exception as e:
        logger.error(f"Error while approving request for {file_name}: {e}", exc_info=True)
        try:
            bot.send_message(chat_id, f"❌ Error starting hosting: {e}")
        except Exception:
            pass

    try:
        bot.edit_message_text(f"✅ Approved: `{file_name}` (User `{target_uid}`)", call.message.chat.id, call.message.message_id, parse_mode="Markdown")
    except Exception:
        pass
...
def handle_callbacks(call):
    ...
    try:
        ...
        elif data.startswith('approve_') or data.startswith('reject_'): handle_approval_callback(call)
